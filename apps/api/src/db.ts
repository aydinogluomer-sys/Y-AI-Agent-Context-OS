...
import { runMigrations as runFileMigrations } from "@y/db";
...
export class DatabaseConnector {
   ...
   * Migration sequencer.
   *
   * P01 / ADR-003: Migration SQL'i artik bu dosyada YASAMIYOR.
   * `migrations/NNNN_*.sql` dosyalarindan `@y/db` runner'i tarafindan
   * okunur. Bu metot yalnizca baglanti sahipligini ve durum guncellemesini
   * yonetir.
   *
   * Onceki hali 1.160 satirlik inline bir string diziydi; sema
   * gozden gecirilemez, diff'lenemez ve araclarla dogrulanamaz durumdaydi.
   * Ledger anahtarlari (schema_migrations.version) degismedi, bu yuzden
   * halihazirda migrate edilmis veritabanlari etkilenmez.
   */
  public async runMigrations(): Promise<{ migrated: boolean; processedVersions: string[] }> {
    if (!this.pool) {
    ...
    try {
      const result = await runFileMigrations(this.pool, {
        migrationsDir: MIGRATIONS_DIR,
        logger: (message) => sysLogger.info(message)
      });

      sysLogger.info(
        `Database migrations checked. Applied: ${result.applied.length}, already present: ${result.skipped.length}, total: ${result.total}.`
      );
      ...
      return {
        ...
        processedVersions: result.applied
      };
    } catch (error: any) {
      sysLogger.error(
        `FATAL: Database migrations failed! Transaction rolled back. Error: ${error.message}`
      );
      throw error;
    }
  }
