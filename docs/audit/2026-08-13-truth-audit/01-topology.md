...
| API router | **6653 satır**, tek `Router()`, 195 route inline | `apps/api/src/index.ts` |
| Web app | **971 satır** tek component, router yok | `apps/web/src/App.tsx` |
| Şema | **Hiç `.sql` dosyası yok** (57 adet) — 1105 satırlık `db.ts` içinde inline string | `apps/api/src/db.ts` |
...
| `packages/security` | 6 | 1253 | wired (30 importer) |
| `packages/shared` | 10 | 3290 | wired (47 importer) |

## Sonuç

- `apps/api/src/index.ts` tek başına 6653 satır ve 195 route taşıyor;
